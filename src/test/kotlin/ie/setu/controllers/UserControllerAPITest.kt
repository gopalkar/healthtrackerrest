    ...
    inner class TestActivities {
        ...
        inner class CreateActivities {
            ...
            fun `add a activity with correct details returns a 201 response`() {
                ...
                val addActivityResponse = addActivity(validActivity, validDuration, validCalories, DateTime.now(), retrievedUser.id)
                ...
                //Assert - verify the contents of the retrieved user
                assertEquals(validActivity, retrievedActivity.description)
        ...
        //helper function to retrieve a test user from the database by email
        private fun retrieveActivityByUser(email: String): HttpResponse<String> {
            return Unirest.get(origin + "/api/activities/email/${email}").asString()
        }
        ...
        //helper function to add a test user to the database
        private fun updateActivity(description: String, duration: Double, calories: Int, started: DateTime, id: Int): HttpResponse<JsonNode> {
